...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  // filter references that have unique urls
  const uniqueReferences = references?.filter(
    (reference, index, self) =>
      index === self.findIndex((t) => t.url === reference.url)
  );
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-4`}>
      <div
        className={`${isMobile ? "max-w-[95%]" : "max-w-[80%]"} ${
          isUser ? "bg-[#283691] text-white" : "bg-gray-100 border"
        } rounded-xl p-4 shadow`}
      >
        <div className="flex items-start gap-3">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              isUser ? "bg-[#1A2670]" : "bg-blue-100"
            }`}
          >
            {isUser ? (
          ...
          <div className="flex-1">
            ...
            {uniqueReferences && uniqueReferences.length > 0 && (
              <div className="mt-5 pt-4 border-t border-gray-200">
                <h4 className="font-semibold text-[#283691] mb-3">
                  Referências Encontradas
                </h4>
                <div className="space-y-4">
                  {uniqueReferences.map((article, index) => (
