...
export const mockArticlesDatabase: Article[] = [
  ...
  },
  {
    id: "3",
    title:
      "Sustentabilidade Empresarial como Vantagem Competitiva no Mercado Brasileiro",
    ...
    snippet:
      "Empresas com forte compromisso com práticas sustentáveis apresentaram melhores índices de valorização de mercado e maior fidelização de clientes no período analisado.",
    keywords: ["sustentabilidade", "vantagem competitiva", "ESG"],
  },
  {
    id: "4",
    title:
      "Transformação Digital em Pequenas e Médias Empresas: Um Estudo Longitudinal",
    ...
    snippet:
      "As PMEs que adotaram estratégias estruturadas de transformação digital apresentaram crescimento médio de receita 37% superior às que implementaram mudanças tecnológicas de forma pontual e não integrada.",
  ...
  },
  {
    ...
    snippet:
      "O estudo revelou que 78% dos profissionais em regime de teletrabalho reportaram aumento de produtividade, embora 42% tenham indicado dificuldades relacionadas ao equilíbrio entre vida pessoal e profissional.",
    keywords: [
      "trabalho remoto",
      "produtividade",
      "bem-estar",
  ...
  },
  {
    ...
    snippet:
      "Os negócios de impacto social cresceram 35% nos últimos três anos no Brasil, com destaque para iniciativas voltadas à educação, saúde e tecnologias ambientais.",
  ...
  },
  {
    ...
    snippet:
      "A pesquisa identificou que a estratégia de internacionalização mais eficaz para startups brasileiras envolve parcerias estratégicas com players locais nos mercados-alvo.",
    keywords: ["internacionalização", "startups", "estratégia empresarial"],
  },
  {
    id: "8",
    title:
      "Ecossistemas de Inovação: O Papel das Universidades no Desenvolvimento de Novos Negócios",
    ...
    snippet:
      "As parcerias entre universidades e setor privado resultaram na criação de 270 novas empresas nos últimos cinco anos, com taxa de sobrevivência 22% superior à média nacional.",
...
export const mockQueryArticles = async (query: string): Promise<Article[]> => {
  ...
  let matchedArticles = mockArticlesDatabase.filter((article) => {
    ...
    const snippetMatch = article.snippet
      ?.toLowerCase()
  ...
  if (matchedArticles.length === 0) {
    ...
    for (const [topic, relatedWords] of Object.entries(topics)) {
      ...
      const isRelated = queryWords.some(
        (word) => word === topic || relatedWords.includes(word)
      );

      if (isRelated) {
        matchedArticles = mockArticlesDatabase.filter(
          (article) =>
            article.keywords.includes(topic) ||
            article.title.toLowerCase().includes(topic) ||
            relatedWords.some((related) =>
              article.title.toLowerCase().includes(related)
            )
        );
...
export const generateEnhancedAIResponse = (
  query: string,
  articles: Article[]
): string => {
  ...
  const yearRange =
    minYear === maxYear ? `${minYear}` : `${minYear} a ${maxYear}`;
  ...
  if (isQuestion) {
    response += `Analisando os estudos publicados no EMPRAD, posso responder que ${generateQuestionResponse(
      query,
      articles
    )}.\n\n`;
  } else if (isComparison) {
    response += `Os artigos do EMPRAD apresentam diferentes perspectivas sobre este tema. ${generateComparisonResponse(
      query,
      articles
    )}.\n\n`;
  } else if (isDefinition) {
    response += `Segundo a literatura acadêmica do EMPRAD, ${generateDefinitionResponse(
      query,
      articles
    )}.\n\n`;
    ...
    response += `A análise dos artigos revela tendências convergentes sobre ${topKeywords.join(
      ", "
    )} no contexto empresarial brasileiro. Os estudos demonstram a interconexão desses temas e sua relevância para o desenvolvimento de negócios mais competitivos e sustentáveis no atual cenário econômico e social.\n\n`;
  }

  // Generate methodology insights
  response += `**Abordagens metodológicas:**\n`;
  response += `Os estudos analisados utilizaram predominantemente ${generateMethodologyInsights(
    articles
  )}, o que permite compreender tanto as particularidades dos casos estudados quanto estabelecer correlações estatisticamente significativas entre variáveis de interesse.\n\n`;

  // Generate conclusion and next steps
  response += `**Implicações práticas:**\n`;
  response += `As pesquisas sugerem que ${generatePracticalImplications(
    query,
    articles
  )}. A literatura recomenda a adoção de práticas colaborativas e o monitoramento contínuo do ambiente competitivo para identificar oportunidades de inovação e crescimento sustentável.\n\n`;
...
function generateQuestionResponse(query: string, articles: Article[]): string {
  ...
  if (queryLower.includes("como") && queryLower.includes("empreender")) {
    ...
    queryLower.includes("desafio") ||
    queryLower.includes("barreira")
  ) {
...
function generateComparisonResponse(
  ...
  if (queryLower.includes("startup") && queryLower.includes("tradicional")) {
    ...
    queryLower.includes("nacional") &&
    queryLower.includes("internacional")
  ) {
...
function generateDefinitionResponse(
  query: string,
  articles: Article[]
): string {
  const queryLower = query.toLowerCase();
...
function generateMethodologyInsights(articles: Article[]): string {
  ...
  return methodologyOptions[
    Math.floor(Math.random() * methodologyOptions.length)
  ];
}
...
export async function getArticleSummary(
  ...
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/articles/summary/${articleId}`
  );
